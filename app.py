...
// ==UserScript==
// @name         Portal to NotebookLM Auto-Automation
// @namespace    http://tampermonkey.net/
// @version      1.0
// @description  Automatically catch text commands from the portal and type them into NotebookLM
// @author       You
...
(function() {
    ...
    // Wait for the NotebookLM page interface to load completely
    window.addEventListener('load', function() {
        const urlParams = new URLSearchParams(window.location.search);
        const portalQuery = urlParams.get('portalQuery');

        if (portalQuery) {
            // Decode the command sent from your app
            const clearCommand = decodeURIComponent(portalQuery);

            // Periodically check for Google's chat textarea element to appear
            let checkExist = setInterval(function() {
                // Target Google's specific text input field or contenteditable element
                let chatTextBox = document.querySelector('textarea, [contenteditable="true"]');
                let submitButton = document.querySelector('button[aria-label="Send message"], button[type="submit"]');

                if (chatTextBox) {
                    clearInterval(checkExist);
                    
                    // Simulate a real user typing the command
                    chatTextBox.focus();
                    chatTextBox.value = clearCommand;
                    if(chatTextBox.innerText !== undefined) { chatTextBox.innerText = clearCommand; }
                    
                    // Trigger input event so Google's React/Angular framework detects the text
                    chatTextBox.dispatchEvent(new Event('input', { bubbles: true }));

                    // Automatically click submit after 1 second
                    setTimeout(() => {
                        if (submitButton) {
                            submitButton.click();
                        }
